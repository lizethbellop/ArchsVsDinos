...
namespace ArchsVsDinosClient.ViewModels
{
    public class UpdateInstagramViewModel
    {
        ...
        public UpdateInstagramViewModel(
        ...
        public async Task SaveInstagramLink()
        {
            if (!SocialMediaValidator.IsValidInstagramLink(NewInstagramLink))
            {
                messageService.ShowMessage(
                    SocialMediaValidator.GetValidationErrorMessage(SocialMediaPlatform.Instagram)
                );
                return;
            }
            ...
            try
            {
                var result = await resetHelper.ExecuteAsync(
                    profileService,
                    client => client.UpdateInstagramAsync(currentUsername, NewInstagramLink)
                );
